...
const mkAttrs = require("markdown-it-attrs")
...
gulp.task("jsPages", function () {
  return gulp.src(paths.src.js.files).pipe(gulp.dest(paths.dist.js.files))
})
...
gulp.task("jsPages:prod", function () {
  return gulp
  ...
  return gulp
    ...
    .pipe(
      fileinclude({
        ...
        indent: true,
        filters: {
          markdown: (arg) => markdown.render(arg),
        },
      })
    )
    .pipe(cached())
